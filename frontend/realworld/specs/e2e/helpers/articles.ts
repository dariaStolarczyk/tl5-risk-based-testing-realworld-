...
export async function favoriteArticle(page: Page) {
  const favoriteButton = page.locator('button:has-text("Favorite")').first();
  const unfavoriteButton = page.locator('button:has-text("Unfavorite")').first();

  // Idempotent: if the article is already favorited, do nothing.
...
export async function unfavoriteArticle(page: Page) {
  ...
  // Idempotent: if the article is already not favorited, do nothing.
