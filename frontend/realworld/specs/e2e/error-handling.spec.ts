...
test.describe('Error Handling - 422 Validation Error', () => {
  test('E2E-04: should render 422 validation errors on article creation', async ({ page }) => {
    await page.route(`${API_BASE}/user`, route => {
      if (route.request().method() === 'GET') {
    ...
    await page.fill('input[name="title"]', '');
    await page.fill('input[name="description"]', 'Validation error test');
    await page.fill('textarea[name="body"]', '');

    const articleResponse = page.waitForResponse(
      response => response.url() === `${API_BASE}/articles` && response.request().method() === 'POST',
    );
    ...
    await expect(errorMessages).toContainText('title is required');
    await expect(errorMessages).toContainText('body is required');
